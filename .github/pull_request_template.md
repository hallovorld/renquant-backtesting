...
- [ ] **This repo owns the promotion gate.** If this PR touches `wf_gate/`, say which SCOPE it affects (`walkforward_manifest` vs `static_artifact`) and whether `candidate_artifact_used` changes. A gate that admits on recipe hash alone is blind to data and universe changes (issues #83, #84).
